...
export default (value: number, options: numberToWordOptionTypes = optionsType) => {
  ...
  const [decimalValue, fractionValue]: string[] = number.split('.');
    ...
    const n: string[] = ('000000000000000' + decimalValue).substr(-15).match(/^(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})(\d{1})(\d{2})$/);
  ...
  const { LESS_THAN_TWENTY:a, TENTHS_LESS_THAN_HUNDRED:b, QUADRILLION_TO_HUNDRED:d } = options;
    ...
    if (fractionValue)
    {
        const coin: any = fractionValue.length === 1 ? fractionValue + '0' : fractionValue;
